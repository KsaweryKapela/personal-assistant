            ...
            "parameters": {
                ...
                "properties": {
                    "message": {
                        "type": "string",
                        "description": (
                            "The internal prompt/instruction the AI will receive at the scheduled time. "
                            "It MUST be a complete, self-sufficient directive — the AI will have no memory "
                            "of the current conversation when this fires. "
                            "Always include: the specific topic (e.g. 'work block', 'gym session'), "
                            "what to ask or say, and the tone. "
                            "Good: '[CHECK-IN] Ask the user how their afternoon work block went. Were they "
                            "productive? How are they feeling? Keep it warm and brief.' "
                            "Bad: 'check in', 'scheduled', 'ask about work'. "
                            "The AI will craft the actual user-facing message from this directive."
                        ),
                    },
                    "delay_minutes": {
                        "type": "integer",
...
def run_agent(user_message: str, chat_id: int = 0, request_id: str = "", message_type: str = "text") -> str:
    ...
    system_prompt = (
        ...
        "Use frequency='daily' for every day, frequency='weekdays' for Mon–Fri only, "
        "frequency='weekly' + days_of_week for specific days (e.g. ['MO','WE','FR']). "
        "Always set recurrence_until or recurrence_count unless the user explicitly wants it to repeat forever. "
        ...
        "Do NOT proactively schedule check-ins just because none are scheduled — only schedule when there is a "
        ...
        "When scheduling a check-in, the 'message' field MUST be a full, specific directive that captures "
        "exactly what the check-in is about. Write it as a standalone instruction the AI will receive cold, "
        "with no other context — it must be self-sufficient. "
        "Good example: '[CHECK-IN] Ask the user how their afternoon work block went. Were they productive? "
        "How are they feeling? Keep it warm and brief.' "
        "Bad example: 'check in' or 'scheduled' or 'ask about work'.\n"
        ...
        "   - Every task completed today: acknowledge it and ask for any notes or how it went.\n"
        "   - Every pending/overdue task due today: ask whether it was completed and what happened.\n"
        "Do not skip completed tasks — the user wants to review all of them, done or not. "
        "Group related items if it keeps things concise.\n"
        ...
        f"- {DAILY_MORNING_CHECK_TIME}: morning check-in — asks about sleep quality, morning routine completion, and plans for today. Logs morning routine habit.\n"
