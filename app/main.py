    ...
    if TELEGRAM_CHAT_ID:
        ...
        add_recurring_daily_job(
            ...
            message=(
                ...
                f"Step 3: Identify anything that reveals a stable trait, preference, habit, goal, "
                f"recurring pattern, opinion, or personality characteristic about the user. "
                f"Store ONLY things that describe who the user IS — their values, tendencies, "
                f"routines, preferences, goals, health patterns, strengths, weaknesses, etc. "
                f"DO NOT store one-off events or biographical facts (e.g. 'woke up at 5am on March 5th', "
                f"'had a bad meeting today') — those belong in activities/daily summaries, not the profile. "
                f"A fact is worth storing only if it would still be true and useful to know weeks from now.\n"
                f"Step 4: For each qualifying insight, call update_user_profile. "
                f"Use search_memory first to confirm it isn't already captured before adding it.\n"
                f"Step 5: Clean up the profile — remove or merge any duplicate, redundant, or contradictory "
                f"entries you find. If two fields say the same thing, keep the more specific one.\n"
                f"Step 6: Send the user a short, direct summary — what was added, what was cleaned up, "
                f"and what was skipped and why. Be specific."
            ),
        )
        ...
        add_recurring_daily_job(
            ...
            message=(
                ...
                f"- metadata: any extra structured data worth keeping (e.g. notable events, context) as a JSON object\n\n"
                f"Step 5: Call save_daily_summary passing ALL of the above fields — "
                f"wake_time, sleep_time, sleep_duration_hours, activities_completed, activities_skipped, "
                f"activities_partial, activities_total, completion_rate_pct, workout_done, deep_work_minutes, "
